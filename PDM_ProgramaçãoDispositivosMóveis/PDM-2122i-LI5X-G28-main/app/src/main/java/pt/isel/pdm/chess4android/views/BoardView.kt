...
class BoardView(private val ctx: Context, attrs: AttributeSet?) : GridLayout(ctx, attrs) {
    ...
    fun drawPiece(col: Int, row: Int, player: Army, piece: Piece) {
        val i : Tile = get(row*columnCount+col) as Tile
        i.piece = Pair(player,piece)
    }

    fun deletePiece(col: Int, row: Int){
        val i : Tile = get(row*columnCount+col) as Tile
        i.piece = null
    }
