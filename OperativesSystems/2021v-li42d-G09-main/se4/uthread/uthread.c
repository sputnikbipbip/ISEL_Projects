...
//
// The sentinels of the circular lists linking the user threads that are
// currently schedulable. The next thread to run is retrieved from the
// head of this list.
//
static list_entry_t ready_queue_priority0;//list only for main thread
static list_entry_t ready_queue_priority1;
static list_entry_t ready_queue_priority2;
static list_entry_t ready_queue_priority3;
...
*/
static inline uthread_t * extract_next_thread_priority(){
	
	if(!is_list_empty(&ready_queue_priority3))
		return container_of(remove_list_first(&ready_queue_priority3), uthread_t, entry);
		
	 
	if(!is_list_empty(&ready_queue_priority2))
		return container_of(remove_list_first(&ready_queue_priority2), uthread_t, entry);	
	 
	
	if(!is_list_empty(&ready_queue_priority1))
		return container_of(remove_list_first(&ready_queue_priority1), uthread_t, entry);
		
	//return main_thread
	return container_of(remove_list_first(&ready_queue_priority0), uthread_t, entry);	
}
...
static inline void schedule () {
    ...
    next_thread = extract_next_thread_priority();
...
//
void ut_init() {
	init_list_head(&ready_queue_priority0);
	init_list_head(&ready_queue_priority1);
	init_list_head(&ready_queue_priority2);
	init_list_head(&ready_queue_priority3);
}
...
void ut_run() {
	...
	// 		ready_queue_priority0 - only have the main_thread
	//

	if (is_list_empty(&ready_queue_priority1) && is_list_empty(&ready_queue_priority2)
			&& is_list_empty(&ready_queue_priority3)) {
		return;
	}
	...
	//insert main thread in the ready_queue_priority0
	insert_list_last(&ready_queue_priority0, &(running_thread->entry));
	...
	//
	assert(is_list_empty(&ready_queue_priority1));
	assert(is_list_empty(&ready_queue_priority2));
	assert(is_list_empty(&ready_queue_priority3));
...
//
void ut_yield() {
	//check if exists any user thread left
	if (!is_list_empty(&ready_queue_priority1) || !is_list_empty(&ready_queue_priority2) 
			|| !is_list_empty(&ready_queue_priority3)) {
				
		running_thread->state = Ready;
		
		if(running_thread->p == LOW){
			insert_list_last(&ready_queue_priority1, &(running_thread->entry));
		}else if(running_thread->p == NORMAL){
			insert_list_last(&ready_queue_priority2, &(running_thread->entry));
		}else if(running_thread->p == HIGH){
			insert_list_last(&ready_queue_priority3, &(running_thread->entry));	
		}
		schedule();
	}
}
...
void ut_activate(uthread_t * thread) {
	...
	if(thread->p == IDLE){	//redudant call, only for consistency
		insert_list_last(&ready_queue_priority0, &(thread->entry));
	}else if(thread->p == LOW){
		insert_list_last(&ready_queue_priority1, &(thread->entry));
	}else if(thread->p == NORMAL){
		insert_list_last(&ready_queue_priority2, &(thread->entry));
	}else if(thread->p == HIGH){
		insert_list_last(&ready_queue_priority3, &(thread->entry));	
	}
}
...
//
uthread_t* ut_create(void (*start_routine)(void *), void * arg, int p) {
	assert(p >= 1 && p <= 3);
	uthread_t * thread;
